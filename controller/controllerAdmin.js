...
controller.MenentukanDosen = async (req, res) => {
    ...
    res.render("admin/pilihdosen", {dosen, matkul, rps, dosenMatkul });
}
...
controller.laporan = async (req, res) => {
    ...
    const matkul = await model.course_plans.findAll({attributes:['id', 'rev', 'code', 'name', 'credit']});
    // const id = req.params.id
    // const matkul = await model.courses.findOne({ where: { id: id }})

    res.render('admin/laporanrps', {projectBase, caseBase,cpmk,matkul});
}
